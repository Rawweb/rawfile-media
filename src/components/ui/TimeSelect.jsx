...
function TimeSelect({
  ...
  const options = [{ value: '', label: 'Pick a time' }, ...times];

  // use placeholder if no value selected
  const selected = options.find(t => t.value === value) ?? options[0];
  ...
  return (
    <label className="block">
      ...
      <div className="relative z-30">
        <Listbox
          value={selected}
          onChange={t => onChange({ target: { name, value: t.value } })}
          disabled={disabled}
        >
          <Listbox.Button
          ...
          >
            <Listbox.Options
              className="absolute left-0 right-0 z-30 mt-2 max-h-64 overflow-auto rounded-lg pr-1
            ...
            >
              {options.map(t => (
                <Listbox.Option
                  key={t.value || 'placeholder'}
                  value={t}
                  disabled={t.value === ''} // disable placeholder in dropdown
                  className={({ active }) =>
                    `cursor-pointer select-none px-3 py-2 text-sm ${
                      active ? 'bg-purple-600/20' : ''
                    } ${t.value === '' ? 'text-gray-400' : ''}`
                  }
                >
                  {({ selected }) => (
